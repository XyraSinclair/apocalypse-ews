...
function detect(watch, db, settings, now = Date.now()) {
  ...
  for (const bucket of buckets.values()) {
    ...
    events.push(buildCbrnEvent({ kind: KIND.LEXICAL_BURST, level, occurredAt: new Date(now).toISOString(), title: `CBRN vocabulary burst: ${bucket.region}`, message: `${bucket.region}: ${bucket.term_count} matched surface forms across ${bucket.post_count} distinct matched observations this UTC hour vs a 14-day same-hour median of ${stats.median} (${samples.length} historical samples, robust z ${z.toFixed(2)}, scale floor 1.0). Threshold: terms >= 8 and >= 3x median, matched observations >= 3, z > 0. Source: ${bucket.stream}, one platform or index, partial coverage; ${bucket.stream === 'gdelt-reporting' ? 'place = publisher country' : 'place = name mentioned in the title'}.`, source: bucket.stream, keyParts: [bucket.stream, bucket.region, bucket.bucket_start].map(encodeURIComponent), payload: { stream: bucket.stream, region: bucket.region, bucket_start: bucket.bucket_start, term_count: bucket.term_count, post_count: bucket.post_count, terms: JSON.parse(bucket.terms_json), baseline_median: stats.median, baseline_samples: samples.length, baseline_days: 14, z, scale_floor: 1, minimum_posts: 3, minimum_terms: 8, median_multiplier: 3, high_terms: 25, high_posts: 8 } }));
