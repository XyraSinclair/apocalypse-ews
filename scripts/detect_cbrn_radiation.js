...
function detect(db, settings, now = Date.now()) {
  ...
  for (const run of db.prepare("SELECT * FROM cbrn_ingest_runs WHERE source IN ('de','eurdep') AND consecutive_failures >= 3").all()) {
    ...
    events.push(buildCbrnEvent({ kind: KIND.RADIATION_NETWORK, level: 1, occurredAt: run.last_attempt_at, title: `${run.source} radiation network unavailable`, source: run.source, keyParts: [run.source, run.last_success_at || 'never'], payload: { ...detail, consecutive_failures: run.consecutive_failures, fusion_eligible: false }, message: `${run.source} has failed, returned partial coverage or gone stale for ${run.consecutive_failures} consecutive polls: ${run.last_error}. Station shortfall ${detail.shortfall ?? 'unknown'} of ${detail.roster ?? 'unknown'}; returned ${detail.returned_known ?? 'unknown'}. This is monitoring blindness, not an all-clear. Restored fresh coverage would change this assessment.` }));
