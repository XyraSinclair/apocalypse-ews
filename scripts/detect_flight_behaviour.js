...
function detect(db, options, eventsDb = null) {
  ...
  for (const fixes of byAircraft.values()) {
    ...
    for (let index = 2; index < fixes.length; index += 1) {
      ...
      emit('flight_turnaround', 1, `${c.hex}:${iso(b.time)}`, c.time, `Aircraft turnaround: ${turn.registration || c.hex}`, `${turn.registration || c.hex}: ${b.observed_at} (${round(b.lat)}, ${round(b.lon)}), ${b.altitude_ft} ft to ${c.observed_at} (${round(c.lat)}, ${round(c.lon)}), ${c.altitude_ft} ft; track bearing changed ${round(change)} degrees (${round(incoming)} to ${round(outgoing)}). A single turnaround has ordinary explanations: weather, diversion, mechanical issues, or passenger plans. Subsequent fixes and other aircraft would change the assessment; this does not identify anyone aboard.`, turn);
    }
  }
  ...
  for (const group of components(currentDepartures)) {
    ...
    emit('takeoff_origin_cluster', level, `${hour}:${group.map((point) => point.hex).sort()[0]}`, end, `${group.length} departures near ${place}`, `${group.length} distinct aircraft departed near ${place} within ${options.minutes} minutes, versus a median of ${baseline.median} over ${baseline.sampleCount} previous days. Hubs have busy hours; this comparison is against the same hour on previous days. Scheduled traffic, weather and airspace restrictions can concentrate departures; persistence and independent operational information would change the assessment. No passenger identity or intent is inferred.`, { count: group.length, baseline_median: baseline.median, baseline_samples: baseline.sampleCount, window_minutes: options.minutes, radius_km: 60, centroid: { lat: round(center.lat, 1), lon: round(center.lon, 1) }, aircraft: group.map(({ hex, registration, lat, lon }) => ({ hex, registration, lat: round(lat), lon: round(lon) })) });
  }
