...
function detect(db, eventsDb, settings, now = Date.now()) {
  ...
  for (const group of groups.values()) {
    ...
    const message = `Independent observations agree ${region ? `in ${region}` : `near ${lat.toFixed(2)}, ${lon.toFixed(2)}`}: ${contributors.map((row) => `${row.kind} observed ${row.observed_at}, ${row.distance_km === null ? 'distance unavailable (same named region)' : `${row.distance_km.toFixed(1)} km from the group centroid`}`).join('; ')}. Coincidence across independent instruments raises our confidence in the observation without establishing its cause. The cause remains unresolved. Further independent measurements and authoritative local assessments would change this assessment.`;
    const event = buildCbrnEvent({ kind: KIND.FUSED, level, occurredAt: observedAt, title: 'Independent CBRN observations agree', message, source: [...new Set(group.map((row) => row.family))].sort().join(', '), keyParts: [groupKey], payload: { region, centroid_lat: lat, centroid_lon: lon, contributors, source_families: [...new Set(group.map((row) => row.family))].sort(), window_minutes: settings.windowMinutes, fusion_eligible: false } });
    events.push(event);
